...
class MovingShapes(Scene):
    ...
    def construct(self):
        ...
        self.play(ApplyMethod(circle.move_to, RIGHT),
                  ApplyMethod(square.shift, RIGHT))
        self.play(ApplyMethod(circle.move_to, RIGHT+UP),
                  ApplyMethod(square.shift, RIGHT+UP))
        self.play(ApplyMethod(circle.move_to, LEFT+UP),
                  ApplyMethod(square.shift, LEFT+UP))
...
class SimpleField(Scene):
    ...
    def construct(self):
        ...
        for point in points:
            ...
            # Create vector and shift it to grid point
            result = Vector(field).shift(point)
            vec_field.append(result)  # Append to list

        # Pass list of vectors to create a VGroup
...
if __name__ == "__main__":
    ...
    for item in module_info.values():
        if item.module == module_name:
            print(item.name)
            os.system("python -m manim manim_tutorial_P37.py %s -l" %
